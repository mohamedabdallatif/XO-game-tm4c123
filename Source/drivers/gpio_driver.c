...
void PortF_Init(void){                                               
  ...
  GPIO_PORTF_CR_R = 0x13;           // allow changes to PF4-PF0       
  GPIO_PORTF_AMSEL_R = 0x00;        // 3) disable analog function
  GPIO_PORTF_PCTL_R = 0x00000000;   // 4) GPIO clear bit PCTL  
  GPIO_PORTF_DIR_R = 0x02;  	      // 5) PF4,PF0 input, PF1 as output   
  GPIO_PORTF_AFSEL_R = 0x00;        // 6) no alternate function
  GPIO_PORTF_DEN_R = 0x13;          // 7) enable digital pins PF4,PF0, PF1       
	//GPIO_PORTF_PUR_R |= 0x11;         // enable pullup resistors on PF0,PF4       
  GPIO_PORTF_IS_R &= ~0x11;        	//EDGE SENSE
  GPIO_PORTF_IBE_R &= ~0x11;       	//SINGLE EDGE
  GPIO_PORTF_IEV_R &= ~0x00;        //FAILING EDGE for PF0, PF4
