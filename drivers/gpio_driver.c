...
void PortE_Init(void){
  ...
  GPIO_PORTE_DIR_R = 0x11;          //  PE0 OUTPUT PE1 OUTPUT for LEDs
  GPIO_PORTE_AFSEL_R = 0x00;        //  no alternate function
  GPIO_PORTE_DEN_R =0x03;        //  enable digital pins PE0,PE1  
	GPIO_PORTE_PUR_R |= 0x03;  	   //enable pullup resistors on PE1
}
...
void PortF_Init(void){                                               
  ...
  GPIO_PORTF_DIR_R = 0x0E;          // 5) PF4,PF0, PF5 as input, PF3,PF2,PF1 output   
  GPIO_PORTF_AFSEL_R = 0x00;        // 6) no alternate function
  GPIO_PORTF_DEN_R = 0x3F;          // 7) enable digital pins PF5-PF0       
	GPIO_PORTF_PUR_R |= 0x31;         // enable pullup resistors on PF5,PF4, PF0       
  GPIO_PORTF_IS_R &= ~0x31;        	//EDGE SENSE
  GPIO_PORTF_IBE_R &= ~0x31;       	//SINGLE EDGE
  GPIO_PORTF_IEV_R &= ~0x11;        //FAILING EDGE for PF0, PF4
  GPIO_PORTF_ICR_R = 0x31;          // CLEAR FLAGS ON PF0,PF4
  GPIO_PORTF_IM_R |= 0x31;    		 	// ARM INTERRUPT PF0,PF4
