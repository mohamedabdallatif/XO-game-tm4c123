...
void PortE_Init(void){
  ...
  GPIO_PORTE_CR_R = (1<<0)|(1<<1)|(1<<2)|(1<<3)|(1<<4)|(1<<5);           // allow changes to PE0,PE1    
  ...
  GPIO_PORTE_AFSEL_R = 0x32;        //  no alternate function
  GPIO_PORTE_DEN_R =0x3F;        //  enable digital pins PE0,PE1  
	GPIO_PORTF_PUR_R |= 0x32;  	   		//enable pullup resistors on PE1
	GPIO_PORTE_IS_R &= ~0x32;              //EDGE SENSE
	GPIO_PORTE_IBE_R &= ~0x32;       //SINGLE EDGE
  GPIO_PORTE_IEV_R &= ~0x32;          //FAILING EDGE
  GPIO_PORTE_ICR_R = 0x32;          // CLEAR FLAGS ON PF0,PF4
	GPIO_PORTE_IM_R |= 0x32;			// ARM INTERRUPT PE1
...
void PortF_Init(void){                                               
  ...
  GPIO_PORTF_DEN_R = 0x11;          // 7) enable digital pins PF5,PF0       
